# FourFingerTap — Distribution Guide

## ⚠️ Why the Mac App Store Won't Work

FourFingerTap **cannot be distributed via the Mac App Store** due to two hard blockers:
...
2. **Sandboxing Incompatibility** — App Store apps must be sandboxed. FourFingerTap needs:
...
xcrun notarytool submit "build/FourFingerTap-2.1.dmg" \
...
xcrun stapler staple "build/FourFingerTap-2.1.dmg"
```
...
```bash
gh release create v2.1 build/FourFingerTap-2.1.dmg --title "FourFingerTap 2.1"
```

#### 2. Get the SHA256

```bash
shasum -a 256 build/FourFingerTap-2.1.dmg
```

#### 3. Create the Cask formula

Create a file `Casks/four-finger-tap.rb`:
```ruby
cask "four-finger-tap" do
  version "2.1"
  sha256 "YOUR_SHA256_HERE"

  url "https://github.com/YOUR_USERNAME/FourFingerTap/releases/download/v#{version}/FourFingerTap-#{version}.dmg"
  name "FourFingerTap"
  desc "4-finger trackpad tap to simulate middle-click and other actions"
  homepage "https://github.com/YOUR_USERNAME/FourFingerTap"

  app "FourFingerTap.app"

  zap trash: [
    "~/Library/Preferences/com.fourFingerTap.app.plist",
  ]
end
```
...
Either:
- **Personal tap:** `brew tap your-username/tap` → users install with `brew install --cask your-username/tap/four-finger-tap`
...
```bash
brew install --cask four-finger-tap
```
...
2. Tag a release: `git tag v2.1 && git push --tags`
